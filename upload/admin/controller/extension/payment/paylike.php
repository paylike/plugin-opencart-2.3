...
class ControllerExtensionPaymentPaylike extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/paylike', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/payment/paylike', 'token=' . $this->session->data['token'], true);
		...
		if( version_compare(VERSION, '2.2.0.0', '>=') ) {
			...
			$this->response->setOutput($this->load->view('extension/payment/paylike.tpl', $data));
		}
